...
class TessObject:
    """TESS lightcurve container. Handles data loading, masking, and binning.
    ...
    downsample : int, optional
        Downsample factor applied after binning.
    """
    ...
    def __init__(self, tic_id, masked_h5_path=None, output_dir=None,
        ...
        # Per-sector state (populated by load_data)
        self.sector = None
        self.x = None
        self.y = None
        self.yerr = None
        ...
        self.raw_cadence = None
        self.binned_cadence = None
    ...
    def plot_raw(self, sector=None, output_dir=None):
        ...
        plt.close(fig)
        print(f"Saved raw lightcurve plot → {fname}\n")


class GPFit:
    """GP fitter for time-series data.
    ...
    def __init__(self, x, y, yerr, label="", output_dir=None):
    ...
    def build_numpyro_model(self, prior_params=None):
        """Build a NumPyro GP model with SHOTerm + RotationTerm kernel.

        Constructs a plain callable that mirrors ``build_pymc_model`` but uses
        NumPyro distributions, ``jnp.exp``, and ``celerite2.jax``.  The model
        is stored in ``self._numpyro_model``.

        .. note::
           ``celerite2.jax`` custom ops only have CPU kernels, so JAX is forced
           to CPU (``JAX_PLATFORMS=cpu``).  The speed-up over PyMC comes from
           JIT compilation and vectorized chains, not GPU execution.
        """
        import jax
    ...
    def find_map(self, sampler="pymc", start=None):
        """Find the MAP solution.

        Builds the model and runs MAP optimization. Stores the result in
        ``self.map_soln``. Prior parameters must be set via
        ``build_pymc_model(prior_params=...)`` before calling this method.

        Parameters
        ----------
        sampler : str
            Backend to use for the model: "pymc" or "numpyro".
        start : dict, optional
            Initial parameter values. If None, defaults are used.
        ...
        if not hasattr(self, "_prior_params") or self._prior_params is None:
            raise ValueError("Prior parameters must be set before finding MAP. Call build_pymc_model(prior_params=...) first.")
        ...
        if sampler == "pymc":
            ...
            # Build the NumPyro model for later sampling, but reuse PyMC MAP
            # optimisation (the MAP solution is just a dict of floats).
            model = self.build_pymc_model()
            self._model = model
            self.map_soln = self._find_map_pymc(model, start=start)
    ...
    def _prepare_init_points(self, map_soln, chains, init_jitter=0.1):
        ...
        Parameters
        ----------
        map_soln : dict or None
            MAP parameter values.  If ``None``, returns ``None``.
        chains : int
            Number of chains (one jittered start per chain).
        init_jitter : float
            Relative jitter scale applied to each MAP value.

        Returns
        -------
        list of dict or None
            One init-point dict per chain, or ``None`` if no MAP provided.
        """
        import pandas as pd
    ...
    def _sample_numpyro(self, model_fn, tune, draws, chains, target_accept,
        ...
        ----------
        model_fn : callable
            NumPyro model function (from ``build_numpyro_model``).
        ...
        map_soln : dict, optional
            MAP solution used to initialise chains.
        step_scale : float, optional
            Initial NUTS step size.
        init_jitter : float
            Relative jitter applied to MAP start points.
        ...
        # Convert list-of-dicts → {name: jnp.array([v0, v1, ...])} for NumPyro
        init_params = None
    ...
    def fit(self, sampler="pymc", tune=2000, draws=1000, chains=4,
            target_accept=0.9, cores=1, chain_method="parallel", map_soln=None,
            callback=None, step_scale=None, init_jitter=0.1):
        """Run MCMC sampling on the GP model.

        Parameters
        ----------
        sampler : str
            Sampler backend: "pymc" or "numpyro".
        tune, draws, chains : int
            MCMC sampling parameters.
        target_accept : float
        ...
        step_scale : float, optional
            Initial step size for NUTS before adaptation.
        map_soln : dict, optional
            MAP solution used to initialise chains. If None, uses prior defaults.
        callback : callable, optional
            Sampling callback (e.g. WalkerPlotCallback).
        init_jitter : float
            Relative jitter applied to MAP start points per chain.
        ...
        if sampler == "pymc":
            ...
            raise NotImplementedError(f"Sampler {sampler!r} is not yet implemented. Use 'sampler=\"pymc\"' or 'sampler=\"numpyro\"'.")
    ...
    def plot_kernel_components(self, output_dir=None, soln=None):
        ...
        if output_dir is None:
            output_dir = os.path.join(self.output_dir, "kernel_component_plots") if self.output_dir else None
        ...
        prot_suffix = f"  (Prot = {prot_map:.3f} d)" if prot_map is not None else ""
        axs[0].set_title(f"{label_prefix}MAP kernel components{prot_suffix}", fontsize=20)
        ...
        if output_dir is not None:
            ...
            fname = os.path.join(output_dir, f"{prefix}kernel_components.png")
            fig.savefig(fname, bbox_inches="tight", dpi=150)
    ...
    def _save_results(self):
        ...
        if self.output_dir is None:
            return
        label_tag = self.label or "gp_fit"

        posterior_dir = os.path.join(self.output_dir, "posterior_results")
        os.makedirs(posterior_dir, exist_ok=True)
        self.trace.to_netcdf(os.path.join(posterior_dir, f"{label_tag}.nc"))

        self.summary = az.summary(self.trace, var_names=self.free_var_names)
        summary_dir = os.path.join(self.output_dir, "summary_stats")
        os.makedirs(summary_dir, exist_ok=True)
        self.summary.to_csv(os.path.join(summary_dir, f"{label_tag}.csv"))
    ...
    def plot_map(self, output_dir=None, return_fig=False, alpha=0.2,
        ...
        if output_dir is None:
            output_dir = os.path.join(self.output_dir, "gp_map_plots") if self.output_dir else None
        ...
        cadence = np.median(np.diff(self.x))
        prot_label = f" (Prot={prot_map:.2f} d)" if prot_map is not None else ""
        for ax in axs:
            ax.errorbar(self.x, self.y, yerr=self.yerr, fmt=".k", capsize=0,
                        label=f"Data (N={len(self.x)} | cadence={cadence*24*60:.1f} min)")
            if x_transit is not None and y_transit is not None:
                ax.plot(x_transit, y_transit, ".C1", ms=4, alpha=0.6, label="Transit points")
            ax.plot(xgrid, mu_map, color="b", lw=1.5, label=f"MAP prediction{prot_label}")
        ...
        title = f"{self.label} (MAP)" if self.label else "GP MAP fit"
        ...
        if output_dir is not None:
            os.makedirs(output_dir, exist_ok=True)
            prefix = f"{self.label}_" if self.label else ""
            fname = os.path.join(output_dir, f"{prefix}map_fit.png")
            fig.savefig(fname, bbox_inches="tight", dpi=150)
            print(f"Saved MAP fit plot → {fname}\n")
        if return_fig:
            return fig, axs
        plt.close(fig)
    ...
    def plot_gp_fit(self, xgrid, mu, var, output_dir=None, mu_map=None, var_map=None,
        ...
        fig, axs = self.plot_map(output_dir=None, return_fig=True, alpha=0.1,
                                 x_transit=x_transit, y_transit=y_transit)

        if self.summary is not None and "period" in self.summary.index:
            prot_label = f" (Prot={float(self.summary.loc['period', 'mean']):.2f} d)"
        else:
            prot_label = ""
        for ax in axs:
            ax.plot(xgrid, mu, color="r", label=f"GP posterior mean{prot_label}")
            ax.fill_between(xgrid, mu - sigma, mu + sigma, color="r", alpha=0.3)

        axs[0].legend(fontsize=12)
        title = self.label if self.label else "GP fit"
        axs[0].set_title(title, fontsize=22)

        if output_dir is None:
            output_dir = os.path.join(self.output_dir, "gp_fit_plots") if self.output_dir else None

        fig.tight_layout()
        if output_dir is not None:
            os.makedirs(output_dir, exist_ok=True)
            prefix = f"{self.label}_" if self.label else ""
            fname = os.path.join(output_dir, f"{prefix}gp_fit.png")
            fig.savefig(fname, bbox_inches="tight", dpi=150)
    ...
    def _plot_results(self):
        ...
        plot_dir = os.path.join(self.output_dir, "plots") if self.output_dir else None
    ...
    def _get_priors(self):
        ...
        Reads from ``self._prior_params`` (set via ``build_pymc_model``).
        ...
        if self._prior_params is None:
            raise RuntimeError(
                "Prior parameters have not been set. "
                "Call build_pymc_model(prior_params=...) first."
            )
    ...
    def plot_priors(self, soln=None, return_fig=False, output_dir=None):
        ...
        for ax, name in zip(axes, names):
            ...
            if soln is not None and name in soln:
                map_val = float(soln[name])
                ax.axvline(map_val, color="C1", lw=2, ls="--",
                           label=f"MAP = {map_val:.4g}")
                ax.legend(fontsize=14, loc="upper right")
            elif self.map_soln is not None and name in self.map_soln:
        ...
        title = f"{self.label} | Prior distributions" if self.label else "Prior distributions"
        fig.suptitle(title, fontsize=14, y=1.01)
        fig.tight_layout()

        if output_dir is None:
            output_dir = os.path.join(self.output_dir, "prior_plots") if self.output_dir else None
        if output_dir is not None:
            os.makedirs(output_dir, exist_ok=True)
            prefix = f"{self.label}_" if self.label else ""
            fname = os.path.join(output_dir, f"{prefix}priors.png")
            fig.savefig(fname, bbox_inches="tight", dpi=150)
    ...
    def plot_corner(self, output_dir=None, show_priors=True):
        ...
        title = self.label if self.label else "GP posterior"
        fig.suptitle(title, fontsize=20, y=1.02)

        if output_dir is None:
            output_dir = os.path.join(self.output_dir, "corner_plots") if self.output_dir else None
        if output_dir is not None:
            os.makedirs(output_dir, exist_ok=True)
            prefix = f"{self.label}_" if self.label else ""
            fname = os.path.join(output_dir, f"{prefix}corner.png")
            fig.savefig(fname, bbox_inches="tight", dpi=150)
            print(f"Saved corner plot → {fname}\n")
        plt.close(fig)

    def plot_summary(self, output_dir=None, x_transit=None, y_transit=None):
        ...
        axs[0].scatter(self.x, self.y, color="k", s=3, label="Data")
        if x_transit is not None and y_transit is not None:
            axs[0].scatter(x_transit, y_transit, color="C1", s=3, alpha=0.6, label="Transit points")
        ...
        axs[0].set_title(self.label if self.label else "GP summary", fontsize=18)
        axs[0].legend(fontsize=12)
        axs[0].minorticks_on()
        ...
        if x_transit is not None and y_transit is not None:
            axs[1].scatter(x_transit % prot_mean, y_transit, color="C1", s=3, alpha=0.6)
        ...
        if output_dir is None:
            output_dir = os.path.join(self.output_dir, "summary_plots") if self.output_dir else None
        if output_dir is not None:
            os.makedirs(output_dir, exist_ok=True)
            prefix = f"{self.label}_" if self.label else ""
            fname = os.path.join(output_dir, f"{prefix}gp_summary.png")
            fig.savefig(fname, bbox_inches="tight", dpi=150)
    ...
    def plot_phase_fold(self, period=None, n_bins=50, output_dir=None,
        ...
        if self.x is None or self.y is None:
            raise RuntimeError("No data available. Ensure x, y, yerr were passed to GPFit.__init__.")
        ...
        if period is None:
            ...
            if period is None:
                raise ValueError(
                    "No period specified and none found in solution."
                )

        if output_dir is None:
            output_dir = os.path.join(self.output_dir, "phase_fold_plots") if self.output_dir else None
        if output_dir is not None:
            os.makedirs(output_dir, exist_ok=True)
        ...
        # Transit points
        if show_transit and x_transit is not None and y_transit is not None:
            phase_transit = (x_transit % period) / period
            ax.scatter(phase_transit, y_transit, c="C1", s=3,
        ...
        ax.set_title(f"{title_prefix}Phase-folded (P = {period:.4f} d)", fontsize=18)
        ...
        if output_dir is not None:
            prefix = f"{self.label}_" if self.label else ""
            fname = os.path.join(output_dir, f"{prefix}phase_fold.png")
            fig.savefig(fname, bbox_inches="tight", dpi=150)
    ...
    def reload(self, results_dir=None):
        """Reload saved GP fit results.
        ...
        trace_path = os.path.join(results_dir, "posterior_results", f"{label_tag}.nc")
        self.trace = az.from_netcdf(trace_path)

        summary_path = os.path.join(results_dir, "summary_stats", f"{label_tag}.csv")
    ...
    def plot_psd(self, output_dir=None, return_fig=False, show_kernel=True,
        ...
        if output_dir is None:
            output_dir = os.path.join(self.output_dir, "psd_plots") if self.output_dir else None
        if output_dir is not None:
            os.makedirs(output_dir, exist_ok=True)
            prefix = f"{self.label}_" if self.label else ""
            fname = os.path.join(output_dir, f"{prefix}psd.png")
            fig.savefig(fname, bbox_inches="tight", dpi=150)
    ...
    def _draw_psd(self, ax, soln=None, show_kernel=True,
        ...
        psd_title = f"{self.label} — PSD" if self.label else "PSD"
        ax.set_title(psd_title, fontsize=20)
    ...
    def plot_acf(self, output_dir=None, return_fig=False, max_lag=None,
        ...
        if output_dir is None:
            output_dir = os.path.join(self.output_dir, "acf_plots") if self.output_dir else None
        if output_dir is not None:
            os.makedirs(output_dir, exist_ok=True)
            prefix = f"{self.label}_" if self.label else ""
            fname = os.path.join(output_dir, f"{prefix}acf.png")
            fig.savefig(fname, bbox_inches="tight", dpi=150)
    ...
    def _draw_acf(self, ax, soln=None, show_kernel=True, max_lag=None,
        ...
        acf_title = f"{self.label} — ACF" if self.label else "ACF"
        ax.set_title(acf_title, fontsize=20)
    ...
    def plot_psd_acf(self, output_dir=None, return_fig=False, soln=None, 
        ...
        if output_dir is None:
            output_dir = os.path.join(self.output_dir, "psd_acf_plots") if self.output_dir else None
        if output_dir is not None:
            os.makedirs(output_dir, exist_ok=True)
            prefix = f"{self.label}_" if self.label else ""
            fname = os.path.join(output_dir, f"{prefix}psd_acf{plot_suffix}.png")
            fig.savefig(fname, bbox_inches="tight", dpi=150)
